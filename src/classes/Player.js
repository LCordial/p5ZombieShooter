class Player {
  ...
  playerController() {
    ...
    if (keyIsDown(this.wKey)) {
      if (this.y + -20 > 0) this.yVelocity -= this.acceleration;
    }
    if (keyIsDown(this.aKey)) {
      if (this.y + 20 < height) this.yVelocity += this.acceleration;
    }
    if (keyIsDown(this.sKey)) {
      if (this.x + -20 > 0) this.xVelocity -= this.acceleration;
    }
    if (keyIsDown(this.dKey)) {
      if (this.x + 20 < width) this.xVelocity += this.acceleration;
    }
    ...
    for (let axis = "x"; true; axis = "y") {
      if (
        ...
        this[axis + "Velocity"] > this.drag * -1
      ) {
